...
| `GOOGLE_VERTEX_SERVICE_ACCOUNT_KEY` | Google Service Account key (JSON or base64-encoded JSON) with Vertex AI User role        |          |
| `GOOGLE_VERTEX_USE_ADC`              | Set to `true` to use Application Default Credentials (ADC)                              |          |

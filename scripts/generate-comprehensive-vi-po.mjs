...
const VI_TRANSLATIONS = new Map([
  ...
  [
    ...
    'Kiểm soát quyền hiển thị mặc định của tài liệu được tải lên.',
  ],
  [
    'Controls the default language of an uploaded document. This will be used as the language in email communications with the recipients.',
    'Kiểm soát ngôn ngữ mặc định của tài liệu được tải lên. Ngôn ngữ này sẽ được sử dụng trong các email gửi tới người nhận.',
  ],
  [
    'Controls which signatures are allowed to be used when signing a document.',
    'Kiểm soát các loại chữ ký được phép sử dụng khi ký tài liệu.',
  ],
  [
    'Recipients that will be automatically added to new documents.',
    'Những người nhận sẽ được tự động thêm vào các tài liệu mới.',
  ],
  ...
  [
    'Your email has been successfully confirmed! You can now use all features of Crove Sign.',
    'Email của bạn đã được xác nhận thành công! Bạn có thể sử dụng đầy đủ các tính năng của Crove Sign.',
  ],
  [
    'Your email has already been confirmed. You can now use all features of Crove Sign.',
    'Email của bạn đã được xác nhận trước đó. Bạn có thể sử dụng tất cả tính năng của Crove Sign.',
  ],
  [
    'This document is available in your Crove Sign account. You can view more details, recipients, and audit logs there.',
    'Tài liệu này khả dụng trong tài khoản Crove Sign của bạn. Bạn có thể xem thêm chi tiết, người nhận và nhật ký kiểm toán tại đó.',
  ],
...
for (const entry of entries) {
  ...
  const rawMsgId = entry.msgid.map((l) => JSON.parse(l)).join('');
